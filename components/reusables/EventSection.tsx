    ...
    <div className={styles.Event}>
      ...
      <div className={styles.EventContent}>
        <h4 className={styles.EventContentHeader}>
          Cisco Innovation Labs Challenge: APIClarity Edition
        </h4>
        <div className={styles.EventContentColumnContainer}>
          {cilApiclarityColumn.map((data, i) => {
            return <EventContentColumn columnData={data} key={i} />;
          })}
        </div>
      </div>
      <img
...
const EventContentColumn = ({ columnData }: { columnData: IEventColumn }) => {
  return (
    <div className={styles.EventContentColumn}>
      <p className={styles.EventContentColumnHeader}>{columnData.title}</p>
      <p className={styles.EventContentColumnText}>{columnData.text}</p>
      {columnData.eventDetails ? (
        <>
          <div className={styles.EventContentColumnDivider} />
          {columnData.eventDetails.map((event, i) => (
            <p key={i} className={styles.EventContentColumnDetails}>
              <span>{event.name}</span> {event.date}
            </p>
          ))}
        </>
      ) : null}
      <div className={styles.EventContentColumnButtonContainer}>
        <CustomLink
