...
const TOCSection = () => {
  const { challenges, solution, building, installationk8s } = useDocsContext();
  return (
    <div className={styles.TOCSection}>
      ...
      <p
        onClick={() => installationk8s.current.scrollIntoView()}
        className={styles.TOCSectionTopic}
      >
...
const ContentSection = () => {
  ...
  return (
    <div className={styles.ContentSection}>
      <h3 className={styles.ContentSectionHeader}>APIClarity</h3>
      <p className={styles.ContentSectionSubheader}>
        Open source for API traffic visibility in K8s clusters
      </p>
      <TopicSection reference={challenges} header={DocsSections.Challenges}>
        <Challenges />
      </TopicSection>
      <TopicSection reference={solution} header={DocsSections.Solution}>
        <Solution />
      </TopicSection>
      <TopicSection reference={building} header={DocsSections.Building}>
        <Building />
      </TopicSection>
      <TopicSection
...
const TopicSection = ({
  ...
  return (
    <div ref={reference} className={styles.TopicSection}>
      <h4 className={styles.TopicSectionHeader}>{header}</h4>
      {children}
    </div>
  );
};
  ...
  return (
    <div>
      <p className={styles.DocsPageParagraph}>
        Microservice applications interact via API’s with many other
        applications. To minimize risk, it is valuable to have visibility to the
        OpenAPI specifications and to understand any potential changes to that
        specification throughout the application lifecycle. However, obtaining
        OpenAPI specs can be challenging, particularly for external or legacy
        applications.
      </p>
      <p className={styles.DocsPageParagraph}>
        Proper OpenAPI specifications can be further complicated by
        microservices that use deprecated APIs (a.k.a. Zombie APIs)​ or
        microservices that use undocumented APIs (a.k.a. Shadow APIs).
      </p>
      <p className={styles.DocsPageParagraph}>
        Finally, it’s important to be able to obtain Open API specifications
        without code instrumentation or modifying existing workloads.
      </p>
    </div>
...
const Solution = () => {
  return (
    <>
      <ul className={styles.DocsPageList}>
        <li className={styles.DocsPageListItem}>
          Capture all API traffic in an existing environment using a
        ...
        <li className={styles.DocsPageListItem}>
          Allow the User to upload OpenAPI spec, review, modify and approve
          generated OpenAPI specs​
        </li>
        <li className={styles.DocsPageListItem}>
          Alert the user on any difference between the approved API
          specification and the one that is observed in runtime, detects shadow
          & zombie APIs​​
        </li>
        <li className={styles.DocsPageListItem}>
    ...
    <>
      ...
      <pre className={styles.DocsPageCodeBlock}>
        <code>
          ...
          <p className={styles.DocsPageCodeBlockLine}>
            <span className={styles.DocsPageCodeBlockLineCommented}>
              # Modify the image name of the apiclarity deployment in
              ./deployment/apiclarity.yaml
            </span>
          </p>
        </code>
      </pre>
      <p className={styles.DocsPageSectionSubheader}>Building UI</p>
      <pre className={styles.DocsPageCodeBlock}>
        <code>
          <p className={styles.DocsPageCodeBlockLine}>make ui</p>
        </code>
      </pre>
      <p className={styles.DocsPageSectionSubheader}>Building Backend</p>
      <pre className={styles.DocsPageCodeBlock}>
        <code>
          <p className={styles.DocsPageCodeBlockLine}>make backend</p>
        </code>
      </pre>
    </>
  );
    ...
    <>
      <ol className={styles.DocsPageList}>
        <li className={styles.DocsPageListItemOrdered}>
          Make sure that Istio is installed and running in your cluster: (
          <a
        ...
        <li className={styles.DocsPageListItemOrdered}>
          Deploy APIClarity in K8s (will be deployed in a new namespace named
          apiclarity):
        </li>
        <pre className={styles.DocsPageCodeBlock}>
          <code>
        ...
        <pre className={styles.DocsPageCodeBlock}>
          <code>
            ...
            <div className={styles.DocsPageCodeBlockLineGrid}>
              <span>NAME</span>
              <span>READY</span>
              <span>STATUS</span>
              <span>RESTARTS</span>
        ...
        <pre className={styles.DocsPageCodeBlock}>
          <code>
            ...
            <p className={styles.DocsPageCodeBlockLine}>cd wasm-filters</p>
          </code>
        </pre>
        <li className={styles.DocsPageListItemOrdered}>
          Run the WASM deployment script for selected namespaces to allow
          traffic tracing. The script will automatically:
        </li>
        <ul>
          <li className={styles.DocsPageListItem}>
          ...
          <li className={styles.DocsPageListItem}>
            Patch all deployment annotations within the selected namespaces to
            mount the WASM binary.
          </li>
        </ul>
        <pre className={styles.DocsPageCodeBlock}>
          <code>
        ...
        <p className={styles.DocsPageParagraph}>
          Note: To build the WASM filter instead of using the pre-built binary,
        ...
        <li className={styles.DocsPageListItemOrdered}>
          ...
          <a className={styles.DocsPageLink} href="http://localhost:9999">
            http://localhost:9999
          </a>
        </li>
        <li className={styles.DocsPageListItemOrdered}>
      ...
      <ol className={styles.DocsPageList}>
        ...
        <li className={styles.DocsPageListItemOrdered}>Copy the built site:</li>
        <pre className={styles.DocsPageCodeBlock}>
          <code>
            <p className={styles.DocsPageCodeBlockLine}>
              cp -r ./ui/build ./site
        ...
        <li className={styles.DocsPageListItemOrdered}>
          Open APIClarity UI in the browser:{" "}
          <a className={styles.DocsPageLink} href="http://localhost:8080">
            http://localhost:8080
          </a>
        </li>
      </ol>
  ...
  return (
    <>
      <p className={styles.OverviewSectionHeader}>
        ...
        RESTful web services. However, there are a few challenges:
      </p>
      <ul className={styles.OverviewSectionList}>
        <li className={styles.OverviewSectionListItem}>
          Not all applications have an OpenAPI Spec available (legacy, external)
        </li>
        <li className={styles.OverviewSectionListItem}>
          Differences between the OpenAPI Spec and how the application actually
          runs can develop over time. Additionally, there is potential that an
          enterprise will have:
        </li>
        <ul className={styles.OverviewSectionList}>
          <li className={styles.OverviewSectionListItem}>
            Zombie APIs – deprecated API’s that are still running somewhere.
          </li>
          <li className={styles.OverviewSectionListItem}>
            Shadow APIs – undocumented APIs, or those that don’t follow an
            enterprise deployment process.
          </li>
        </ul>
        <li className={styles.OverviewSectionListItem}>
          Obtaining OpenAPI Specifications without code instrumentation or
      ...
      <ul>
        <li className={styles.OverviewSectionListItem}>
          The ability to provide all API traffic in an environment using a
          Service-Mesh framework
        </li>
        <li className={styles.OverviewSectionListItem}>
          An interface to upload an API Spec, review, modify, and approve that
          specification.
        </li>
        <li className={styles.OverviewSectionListItem}>
          By observing all traffic within the Service-Mesh, the ability to
          “re-construct” the OpenAPI specification
        </li>
        <li className={styles.OverviewSectionListItem}>
          Alerting capabilities to notify the user of any differences between
          the approved API specification and the one that is observed in
          runtime, as well as detect shadow and Zombie APIs
        </li>
        <li className={styles.OverviewSectionListItem}>
